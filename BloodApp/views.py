...
def RegisterAction(request):
    if request.method == 'POST':
        ...
        output = "none"
        con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'blood',charset='utf8')
        with con:
            cur = con.cursor()
            ...
            for row in rows:
                if row[0] == username:
                    ...
                    break                
        if output == "none":
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'blood',charset='utf8')
